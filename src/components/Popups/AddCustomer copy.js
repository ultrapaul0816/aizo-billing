...
export default function AddCustomer({ data, onClose, isOpen, callback }) {
  ...
  console.log("mmmmmmmmmmmmmmmmm", customerAddresses)
  useEffect(() => {
  ...
  const checkInputs = () => {
    console.log("tttttttttttuuuuuuuuuuuuu", customerDetails)
    if (customerDetails.customer_opt_out) return true;
    for (let c of Object.keys(customerDetails)) {
      if (customerDetails[c] == "" && c !== "email" && c != "address_type") {
        ...
        <div className="customer-det">
                  ...
                  ? selectedCustomer.order_history.slice(0, 2).map((h) => (
                    <div className="past-orders-card">
                      ...
                      <div className="past-orders-card-s">
                        ...
                        {selectedCustomer.product_detail.map((ob) => (
                          <span key={ob.product_name}>
                            {ob.product_name} x{ob.qty}
                          </span>
                        ))}
          ...
          <div>
            ...
            >
              ...
              >
                <Button
                  ...
                  text={customerDetails?.locality?.area || "Select Locality"}
                  // intent='primary'
                  rightIcon="chevron-down"
                />
              </Select>
            ...
            <InputGroup
              leftIcon="bookmark"
              onChange={handleInput}
              value={customerDetails.address_type}
              name="address_type"
              placeholder="Address Type Eg. Work, Home,etc.."
              multiple
              large
            />
            <InputGroup
